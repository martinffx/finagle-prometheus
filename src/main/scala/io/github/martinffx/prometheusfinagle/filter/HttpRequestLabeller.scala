...
/** ServiceLabeller allows Finagle metrics to be labelled specific to the
...
trait ServiceLabeller[Req, Rep] {
  ...
  /** The keys that metrics should be labelled with.
    * @return
    *   a sequence of key names
    */
  def keys: Seq[String]

  /** For a given request and response, this method will choose a value for each
    * key.
    * @param request
    *   the incoming request to label metrics for
    * @param response
    *   the resulting response to label metrics for
    * @return
    *   a sequence of string values, corresponding to [[keys]]
    */
